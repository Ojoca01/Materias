...
// Estrutura de uma linha (via) da cache
typedef struct {
    uint32_t tag;
    uint8_t valid;
    uint8_t age; // 0 = Mais Recente (MRU), 1 = Mais Antigo (LRU)
    uint32_t block[4];
} CacheLine;
...
// Função para inicializar as caches com 0
void init_caches() {
...
int mem_read(uint32_t addr, int size, uint32_t* val) {
    if (addr >= RAM_BASE && addr + size <= RAM_BASE + RAM_SIZE) {
        ...
        *val = temp;
        ...
        if (addr >= CLINT_MTIME && addr < CLINT_MTIME + 8) { // CORRIGIDO: CLINT_MTIME
...
void access_cache(Cache *c, uint32_t addr, char access_type, char cache_type) {
    if (addr < RAM_BASE || addr >= RAM_BASE + RAM_SIZE) return;

    uint32_t index = (addr >> 4) & 0x7; 
    uint32_t tag = addr >> 7;           
    ...
    if (hit) {
        c->hits++;
        set->lines[way].age = 0;
        set->lines[1 - way].age = 1;

        printf("#cache_mem:%c%ch    0x%08x          line=%d,age=%d,id=0x%07x,block[%d]={0x%08x,0x%08x,0x%08x,0x%08x}\n",
               cache_type, access_type, addr, index,
               set->lines[way].age, set->lines[way].tag, way,
               set->lines[way].block[0], set->lines[way].block[1],
               set->lines[way].block[2], set->lines[way].block[3]);
    } else {
        c->misses++;
        
        printf("#cache_mem:%c%cm    0x%08x          line=%d,valid={%d,%d},age={%d,%d},id={0x%07x,0x%07x}\n",
               cache_type, access_type, addr, index,
               set->lines[0].valid, set->lines[1].valid,
               set->lines[0].age, set->lines[1].age,
               set->lines[0].tag, set->lines[1].tag);

        if (!set->lines[0].valid) way = 0;
        else if (!set->lines[1].valid) way = 1;
        else if (set->lines[0].age == 1) way = 0;
        else way = 1;
        ...
        uint32_t block_base = addr & ~0xF;
        for (int i = 0; i < 4; i++) {
            uint32_t w_addr = block_base + i * 4;
            if (w_addr >= RAM_BASE && w_addr < RAM_BASE + RAM_SIZE) {
                uint32_t r_off = w_addr - RAM_BASE;
                set->lines[way].block[i] = ram[r_off] | (ram[r_off+1] << 8) | (ram[r_off+2] << 16) | (ram[r_off+3] << 24);
            } else {
                set->lines[way].block[i] = 0;
            }
        }
    }
}

void sync_dcache_after_store(uint32_t addr) {
    if (addr < RAM_BASE || addr >= RAM_BASE + RAM_SIZE) return;
    uint32_t index = (addr >> 4) & 0x7;
    uint32_t tag = addr >> 7;
    
    for (int i = 0; i < 2; i++) {
        if (dcache.sets[index].lines[i].valid && dcache.sets[index].lines[i].tag == tag) {
            uint32_t block_base = addr & ~0xF;
            for (int w = 0; w < 4; w++) {
                uint32_t w_addr = block_base + w * 4;
                uint32_t r_off = w_addr - RAM_BASE;
                dcache.sets[index].lines[i].block[w] = ram[r_off] | (ram[r_off+1] << 8) | (ram[r_off+2] << 16) | (ram[r_off+3] << 24);
            }
            break;
        }
    }
...
void print_cache_stats() {
    double d_hit = (dcache.hits + dcache.misses) == 0 ? 0.0 : (double)dcache.hits / (dcache.hits + dcache.misses);
    double i_hit = (icache.hits + icache.misses) == 0 ? 0.0 : (double)icache.hits / (icache.hits + icache.misses);
    
    printf("#cache_mem:dstats                     hit=%.4f\n", d_hit);
    printf("#cache_mem:istats                     hit=%.4f\n", i_hit);
}
...
int execute() {
    ...
    switch(opcode) {
        ...
        case 0x23: { // Store
            ...
            access_cache(&dcache, addr_s, 'w', 'd');
            ...
            sync_dcache_after_store(addr_s);
            break;
        }
